...
2. 次に、「ユーザーの入力本文」で渡された内容に基づいて応答する。
   ...
   - **スキル実行時**: 結果やエラーをユーザーに通知するには `!discord send` コマンドを使用する。
     通常のテキスト出力はログチャンネルに流れるため、ユーザーには見えない。
     ```
     !discord send 処理が完了しました。
     ```
   - **通常モード時**: 通常の文字を出力すると、Discord に送信され、ユーザーの目に入る。
     ```
     処理が完了しました。
     ```
...
* スキル実行時に、メッセージを出力するには `!discord send` コマンドを使う。
   * `!discord send メッセージ` は1行だけ送信する。続く行は send の対象にならない。
   * 複数行を送信するには必ずヒアドキュメントを使うこと。
* (WIP)`media:` だけは例外のインライン記法であり、途中の行頭でも認識される
* `!discord` から始めた場合は、コマンドとして処理される。

⚠️Discordではテーブル記法が表示されないので、リストなどで代替すること
...
* 対象ユーザーとボットのメッセージのみを返す（他ユーザーのメッセージは除外）
...
```
[2024-01-01T12:00:00.000Z] Display Name (username) (1234567890): メッセージ内容 [reactions: 👀✅]
[2024-01-01T12:01:00.000Z] Bot Display (botname) [BOT] (1234567891): ボットの応答
[2024-01-01T12:02:00.000Z] Display Name (username) (1234567892): 未処理メッセージ
```
...
* ボットがオフラインだった間に投稿されたメッセージを後から処理するために init スキルで使用する (それ以外の用途ではデッドロックする可能性がある)
* 処理対象のメッセージは現在のチャンネルから取得される

### メッセージ送信

処理の結果やエラーをユーザーに報告する場合に使用する。
...
* デリミタ（`EOF`）は任意の文字列に変更可能
* デリミタは行頭に単独で記述すること
* send の代わりに reply を使えば返信にできる

#### 指定チャンネルに送信

```
!discord sendto #channel-name <メッセージ>
!discord sendto <#1234567890> <メッセージ>
```

複数行も同様：

```
!discord sendto #general <<EOF
複数行
メッセージ
EOF
```
